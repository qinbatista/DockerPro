...
my_domain_name = "cqhome.qinbatista.com"
my_port = "10022"
def get_desktop():
...
def create_git_repositories(repositories):
	...
	print(f'[success]\tgit clone ssh://{name}@{my_domain_name}:{my_port}/Repositories/{name}/{repositories}.git')
	...
	for index, name in enumerate(file_name_lists):
		...
		for index, repos_name in enumerate(repositories_list):
			if repos_name.rfind(".git")==-1:
				continue
			print(f'	[{repos_name}]\t')
			print(f'	git clone ssh://{name}@{my_domain_name}:{my_port}/Repositories/{name}/{repos_name}')
			print(f'	\t')
	print("------------------------")
